...
from 
(select brand, store, transactions_currency, exchange_currency_code, exchange_currency_rate, cast(refund_id as string) refund_id, date(created_at) refund_date, sum(cast(transactions_amount as numeric)) transactions_amount from {{ ref('ShopifyRefundsTransactions')}} where transactions_amount is not null group by 1,2,3,4,5,6,7) rfnd_tran

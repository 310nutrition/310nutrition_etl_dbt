...
{% for i in results_list %}
    ...
    FROM (select
        ...
        case
            ...
            when lower(sourceMedium) like '%email%' then 'Email'
            when lower(sourceMedium) like '%sms%' then 'sms'
            when lower(sourceMedium) like '%referral%' or lower(sourceMedium) like '%social%' or lower(sourceMedium) like '%influencer%' then 'Referral'
        ...
        _daton_batch_id,
        dense_rank() over(partition by date,sourceMedium order by _daton_batch_runtime desc) as row_num
    from {{i}} a)
    where row_num = 1

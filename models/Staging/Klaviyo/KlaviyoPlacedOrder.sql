...
{% for i in results_list %}
    ...
    FROM (
        ...
        --attribution.attributed_channel,
        ...
        case when c.from_currency_code is null then event_properties.currency_code else c.from_currency_code end as exchange_currency_code,
        {% else %}
        cast(1 as decimal) as exchange_currency_rate,
        event_properties.currency_code as exchange_currency_code, 
        {% endif %} 
        event_properties.currency_code,
        ...
        DENSE_RANK() OVER (PARTITION BY a.id, attribution.attributed_event_id ORDER BY a._daton_batch_runtime DESC) AS rnk
        FROM {{i}} a
        LEFT JOIN UNNEST (attributes) AS attributes
        LEFT JOIN UNNEST (attributes.event_properties) AS event_properties
        LEFT JOIN UNNEST (event_properties.attribution) as attribution
        LEFT JOIN UNNEST (links) as links
        {% if var('currency_conversion_flag') %}
        left join {{ref('ExchangeRates')}} c on date(cast(attributes.datetime as timestamp)) = c.date 
        and event_properties.currency_code = c.to_currency_code                      
        {% endif %}

...
{% for i in results_list %}
    ...
    From (
        select
        '{{brand}}' as brand,
        case when lower(CampaignName) like '%canada%' then 'CA' else 'US' end as store,
        AccountId,
        AccountName,
        ...
        Spend,
        cast(timeperiod as timestamp) as TimePeriod,
        TitlePart1,
        TitlePart2,
        ...
        DENSE_RANK() OVER (PARTITION BY AccountNumber,AdId,AdTitle,AdType,DeviceOS,devicetype,DeliveredMatchType,BidMatchType,network,TopVsOther,TimePeriod,language order by a.{{daton_batch_runtime()}} desc) row_num
        from {{i}} a  
            {% if var('currency_conversion_flag') %}

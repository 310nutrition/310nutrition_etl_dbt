...
{{set_table_name('%stayai_subscriptions')}} 
{% endset %}  
...
{% for i in results_list %}
    ...
    FROM (
        ...
        '{{store}}' as store,
        id	,	
        REGEXP_REPLACE(a.subscriptionId, '[^0-9]', '') as subscription_id	,	
        customerId	,	
        emailAddress,		
        firstName,		
        lastName,		
        ...
        orderIntervalUnit	,		
         CAST({{ dbt.dateadd(datepart="hour", interval=hr, from_date_or_timestamp="churnedAt") }} as {{ dbt.type_timestamp() }})  as churnedAt	,		
        CAST({{ dbt.dateadd(datepart="hour", interval=hr, from_date_or_timestamp="cancelledAt") }} as {{ dbt.type_timestamp() }})  as cancelledAt	,		
        cancellationReason	,		
        price_bn,		
        pausedUntil	,		
        dunningStartedAt,		
        ...
        REGEXP_REPLACE(lineItems.shopifyProductId, '[^0-9]', '') as line_item_product_id,		
        REGEXP_REPLACE(lineItems.shopifyVariantId, '[^0-9]', '') as line_item_variant_id	,		
        ...
        lineItems.unitPrice	as line_item_unit_price,		
        lineItems.subtotalPrice as line_item_subtotal_price	,		
        lineItems.isOneTime	as line_item_is_onetime,
        lineItems.sku	as line_item_sku, --added by akash
        {% if var('currency_conversion_flag') %}
        ...
        DENSE_RANK() OVER (PARTITION BY a.subscriptionId order by a.{{daton_batch_runtime()}} desc) row_num
            from {{i}} a
                {% if var('currency_conversion_flag') %}

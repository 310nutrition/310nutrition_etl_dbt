...
{% for i in results_list %}
    ...
    From (
        ...
        SUBSTRING(customer.resource_name, 11) AS portfolio,
        ...
        metrics.clicks,
        coalesce(metrics.conversions,metrics.all_conversions) as conversions,
        metrics.cost_micros,
        coalesce(metrics.conversions_value,metrics.all_conversions_value) as conversions_value,
        metrics.impressions,
        segments.date,
        {% endif %}
        ...
        {% else %}
            ROW_NUMBER() OVER (PARTITION BY SEGMENTS.date,coalesce(campaign.id,campaign.id_nu) order by {{daton_batch_runtime()}} desc) row_num
        {% endif %}
	    from {{i}} 
            {{unnesting("customer")}}
            {{unnesting("campaign")}}

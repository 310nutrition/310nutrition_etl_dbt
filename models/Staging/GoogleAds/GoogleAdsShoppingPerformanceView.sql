...
{% for i in results_list %}
    ...
    From (
        ...
        {% else %}
            Dense_Rank() OVER (PARTITION BY CUSTOMER.resource_name, SEGMENTS.date, AD_GROUP.id,CAMPAIGN.id,SEGMENTS.product_item_id order by {{daton_batch_runtime()}} desc) row_num
        {% endif %}
	    from {{i}} 
            {{unnesting("CUSTOMER")}}
            {{unnesting("CAMPAIGN")}}
